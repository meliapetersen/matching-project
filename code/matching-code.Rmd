...
```{r}
# match on race as treatment (majority vs minority communities) 
# and seek balance on other SES characteristics 

df <- 
  d %>% 
  select( globg00, rent00, hinc00, pnhwht12, pnhblk12, phisp12, 
              ppov12, punemp12,
              hinc12, hincb12, hincw12, 
              phs12, pcol12,
              pown12, mhmval12 ) %>% 
  filter( globg00 %in% c( "White Black","White","Minority" ) ) %>% 
  mutate( white = as.numeric( globg00 == "White" ), 
            minority = as.numeric( globg00 == "Minority" ), 
            diverse = as.numeric( globg00 != "White" ) )
...
Default is nearest neighbor method. As we can see here it does not work very well. 

```{r}
m1 <- matchit( white ~ hinc12 + phs12 + mhmval12, method = "nearest", discard="none", data = df2 )
...
m2 <- matchit( white ~ hinc12 + phs12 + mhmval12, method = "nearest", discard="both", reestimate=TRUE, data = df2 )
...
m2.2 <- matchit( white ~ hinc12 + phs12 + mhmval12, method = "nearest", 
               discard="both", reestimate=TRUE, caliper=.25, data = df2 )

summary( m2.2 )
# improvement but stil not great
```
