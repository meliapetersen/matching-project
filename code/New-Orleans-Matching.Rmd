...
```{r}
Control <- na.omit(matches$control.group.id) %>% as.numeric

Treatment <- na.omit(matches$treatment.group.id) %>% as.numeric

```

```{r}
control.fips <- df3$id2 [ Control ]
...
```{r}
treatment.fips <- df3$id2 [ Treatment ]
