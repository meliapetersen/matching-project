...
```{r}
matchit.object <- build_matchit_metros( fo, df ) 
```
...
build_color_vectors <-  function( matchit.object, dorling.object )

{

  if( is.null(matchit.object) )
  { plot( 0,0, main="No counterfactual for the treatment available" ) }
...
```{r}
color.vector.object <- build_color_vectors(matchit.object, dorling.object)
```


```{r}

build_colored_map <- function(color.vector.object, dorling.object)
...
color.vector <- rep( "gray", nrow( dorling.object ) )
color.vector[ dorling.object$GEOID %in% control.matched ] <- "blue"
color.vector[ dorling.object$GEOID %in% treated.matched ] <- "darkred"
color.vector[ dorling.object$GEOID %in% treated.unmatched ] <- adjustcolor( "darkred", alpha.f = 0.4 )

border.color <- rep( "gray40", nrow(dorling.object) )
border.color [ dorling.object$GEOID %in% treated.unmatched ] <- "darkred"


# dorling.object$matched.polygons <- color.vector
# plot(dorling.object, col=dorling.object$matched.polygons, border=border.color)
...
```{r}
colored.map <- build_colored_map( color.vector.object, dorling.object )
```
...
for( i in cbsa.list )
{
   ...
   fo <- as.formula( "treatment ~ own00" )
   matchit.model <- build_matchit_metros( fo=fo, df=rodeo.data.i)
   vector.model <- build_color_vectors(matchit.object=matchit.model, dorling.object=dorling.object.i)
   build_colored_map( color.vector.object=vector.model, dorling.object=dorling.object.i )
